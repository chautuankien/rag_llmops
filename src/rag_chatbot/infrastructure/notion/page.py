...
class NotionPageClient:
    ...
    def extract_document(self, document_metadata: NotionDocumentMetadata) -> NotionDocument:
        ...
        blocks = self._retrieve_child_blocks(document_metadata.id)
        content, urls = self._parse_blocks(blocks)
        
        parent_metadata = document_metadata.properties.pop("parent", None)
        if parent_metadata:
            parent_metadata = NotionDocumentMetadata(
    ...
    def _retrieve_child_blocks(self, block_id: str) -> list[dict]:
        ...
        blocks_url = f"https://api.notion.com/v1/blocks/{block_id}/children?page_size=100"
        headers = {
    ...
    def _parse_blocks(self, blocks: list[dict], depth: int = 0):
        content = ""
        urls = []
        for block in blocks:
            ...
            if block_type in {"heading_1", "heading_2", "heading_3"}:
                content += f"# {self._parse_rich_text(block[block_type].get('rich_text', []))}\n\n"
                urls.extend(self._extract_urls(block[block_type].get("rich_text", [])))
            
            elif block_type in {"paragraph", "quote"}:
                content += f"{self._parse_rich_text(block[block_type].get('rich_text', []))}\n"
                urls.extend(self._extract_urls(block[block_type].get("rich_text", [])))
            
            elif block_type in {"bulleted_list_item", "numbered_list_item", "toggle"}:
                content += f"- {self._parse_rich_text(block[block_type].get('rich_text', []))}\n"
                urls.extend(self._extract_urls(block[block_type].get("rich_text", [])))
                ...
                content += f"```\n{self._parse_rich_text(block[block_type].get('rich_text', []))}\n```\n"
                urls.extend(self._extract_urls(block[block_type].get("rich_text", [])))
            
            elif block_type == "equation":
                content += f"$${block[block_type].get('expression', '')}$$\n"

            elif block_type == "image":
                # content += f"[Image]({block[block_type].get('external', {}).get('url', 'No URL')})\n"
                content += f"[Image]({block[block_type].get('file', {}).get('url', 'No URL')})\n"
                ...
                child_blocks = self._retrieve_child_blocks(child_id)
                child_content, child_urls = self._parse_blocks(child_blocks, depth + 1)
            ...
            if (
                block_type != "child_page" and
                "has_children" in block and
                block["has_children"]
            ):
                child_blocks = self._retrieve_child_blocks(block_id)
                child_content, child_urls = self._parse_blocks(child_blocks, depth + 1)
                content += (
    ...
    def _parse_rich_text(self, rich_text: list[dict]) -> str:
        ...
        for segment in rich_text:
            # "href" in Notion rich text indicates a hyperlink URL attached to this text segment.
            # If present, we format the text as a Markdown link.
            if segment.get("href"):
                text += f"[{segment.get('plain_text', '')}]({segment.get('href', '')})"
            else:
                if segment.get("equation"):
                    # If the segment is an inline equation, we format it as LaTeX.
                    text += f"${segment.get('plain_text', '')}$"
                else: 
                    text += segment.get("plain_text", "")
        
        return text
    
    def _extract_urls(self, rich_text: list[dict]) -> list[str]:
...
if __name__ == "__main__":
    ...
    client.extract_document("a5cb24b6-5426-4618-9e82-46b6de1f12f9")